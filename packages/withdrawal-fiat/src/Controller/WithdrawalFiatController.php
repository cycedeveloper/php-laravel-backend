...
namespace Sayedsoft\DexwithdrawalFiat\Controller;
...
use Sayedsoft\DexwithdrawalFiat\Models\UserWallet;
...
use Sayedsoft\DexwithdrawalFiat\Base\Withdraw;
use Sayedsoft\DexwithdrawalFiat\Models\withdrawalFiatTokenSettings;
use Sayedsoft\DexwithdrawalFiat\Requests\WithdrawRequest;
use Illuminate\Support\Facades\Validator;
use Sayedsoft\Dex\WalletDeposit\Models\PaymentWallet;

class withdrawalFiatController extends Controller
{
    ...
    private function setTokenSettings (){
        $this->token_settings = withdrawalFiatTokenSettings::whereTokenId($this->token->id)->first();
    }
    ...
    public function getUserWallets()
    {
        ...
        $user_withdrawalFiats = withdrawalFiat::where('user_id',$user->id)->get();
    
        return response()->json(['message'=>'User wallets!','user'=>$user,'wallets'=>$wallets,'user_withdrawalFiats'=>$user_withdrawalFiats]);
    }
    ...
    public function newRequest (Request $request) {
        ...
        $data = [
            ...
            'saveWalletUrl' => route('withdrawalFiat.storeNewWallet')
        ];
        return view('dex-withdrawalFiat::pages.withdraw',$data);
    }
    ...
    public function confirm(WithdrawRequest $request) {
        ...
        return view('dex-withdrawalFiat::pages.confirm',$data);
    }
        ...
        public function history()
    {
        ...
        $user_withdrawalFiats = withdrawalFiat::where('user_id',$user->id)->get();
        $data = [
            'withdrawalFiats'    => $user_withdrawalFiats,
        ];
        return view('dex-withdrawalFiat::pages.withdrawalFiats',$data);
    }
    ...
    public function store (Request $request) {  
        ...
        try {
            ...
            throw $th;
         //   self::message('danger', $th->getMessage());
           // return redirect()->route('withdrawalFiat.new')->with('status', false);
           return $this->responseJosn($request,false,$th->getMessage(),[],420);
        }

        return response()->json(['message'=>'withdrawalFiat request has been sent!'],200); 
